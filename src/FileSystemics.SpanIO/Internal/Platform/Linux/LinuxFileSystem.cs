...
internal sealed class LinuxFileSystem : IFileSystemPlatform<LinuxFileSystem> {
    private LinuxFileSystem() {
    }

    public static bool Exists(ReadOnlySpan<char> path) {
        return LinuxPathRules.WithNativePath(path, (_, utf8Path) =>
                InteropUnix.lstat(utf8Path, out InteropUnix.LinuxStat linuxStat) == 0 &&
                (linuxStat.st_mode & InteropConstants.S_IFMT) == InteropConstants.S_IFREG);
    }

    public static bool DirectoryExists(ReadOnlySpan<char> path) {
        return LinuxPathRules.WithNativePath(path, (_, utf8Path) =>
                InteropUnix.lstat(utf8Path, out InteropUnix.LinuxStat linuxStat) == 0 &&
                (linuxStat.st_mode & InteropConstants.S_IFMT) == InteropConstants.S_IFDIR);
    }
    ...
    public static SafeFileHandle OpenHandle(ReadOnlySpan<char> path, FileMode mode, FileAccess access, FileShare share, FileOptions options) {
        return LinuxPathRules.WithNativePath(path, (_, utf8Path) => PosixFileSystem.OpenHandleCore(utf8Path, mode, access, share));
    }
    ...
    public static SafeFileHandle OpenHandle(ReadOnlySpan<char> directory, ReadOnlySpan<char> fileName, FileMode mode, FileAccess access, FileShare share) {
        return LinuxPathRules.WithCombinedNativePath(directory, fileName, '/', (_, utf8Path) => PosixFileSystem.OpenHandleCore(utf8Path, mode, access, share));
    }

    public static void Delete(ReadOnlySpan<char> path) {
        LinuxPathRules.WithNativePath(path, (_, utf8Path) => {
    ...
    public static void Copy(ReadOnlySpan<char> source, ReadOnlySpan<char> destination, bool overwrite) {
        ...
        if (InteropUnix.fstat(sourceHandle.DangerousGetHandle().ToInt32(), out InteropUnix.LinuxStat stat) != 0) {
            InteropHelpers.ThrowExceptionForErrno(Marshal.GetLastWin32Error());
        }

        long remaining = stat.st_size;
        int inFd = sourceHandle.DangerousGetHandle().ToInt32();
        int outFd = destHandle.DangerousGetHandle().ToInt32();
        while (remaining > 0) {
            nuint chunk = (nuint)Math.Min(remaining, int.MaxValue);
            nint sent = InteropUnix.sendfile(outFd, inFd, nint.Zero, chunk);
            if (sent < 0) {
                break;
            }
    ...
    public static void Move(ReadOnlySpan<char> source, ReadOnlySpan<char> destination) {
        LinuxPathRules.WithTwoNativePaths(
            source,
            destination,
    ...
    public static UnixFileMode GetUnixFileMode(ReadOnlySpan<char> path) {
        int mode = LinuxPathRules.WithNativePath(path, (_, utf8Path) => GetMode(utf8Path));
    ...
    public static long GetLength(ReadOnlySpan<char> path) {
        return LinuxPathRules.WithNativePath(path, (_, utf8Path) => {
                if (InteropUnix.lstat(utf8Path, out InteropUnix.LinuxStat linuxStat) != 0) {
                    InteropHelpers.ThrowExceptionForErrno(Marshal.GetLastWin32Error());
                }

                return linuxStat.st_size;
            });
    }
    ...
    private static (DateTime CreationTimeUtc, DateTime LastAccessTimeUtc, DateTime LastWriteTimeUtc) GetTimestamps(
        ...
        LinuxPathRules.WithNativePath(path, (_, utf8Path) => {
                ...
                return (
                    DateTime.UnixEpoch.AddSeconds(linuxStat.st_ctime).AddTicks(linuxStat.st_ctime_nsec / 100),
                    DateTime.UnixEpoch.AddSeconds(linuxStat.st_atime).AddTicks(linuxStat.st_atime_nsec / 100),
                    DateTime.UnixEpoch.AddSeconds(linuxStat.st_mtime).AddTicks(linuxStat.st_mtime_nsec / 100));
            });
